...
/** Grid placement (lg+) and content density per tile — turns the equal-size deck into a bento layout. */
const BENTO: Record<string, { size: BentoSize; place: string }> = {
  seasonal: { size: 'large', place: 'lg:col-start-1 lg:col-span-2 lg:row-start-1 lg:row-span-2' },
  espresso: { size: 'wide', place: 'lg:col-start-3 lg:col-span-2 lg:row-start-1 lg:row-span-1' },
  flatwhite: { size: 'small', place: 'lg:col-start-3 lg:col-span-1 lg:row-start-2 lg:row-span-1' },
  cappuccino: { size: 'small', place: 'lg:col-start-4 lg:col-span-1 lg:row-start-2 lg:row-span-1' },
  v60: { size: 'small', place: 'lg:col-start-1 lg:col-span-1 lg:row-start-3 lg:row-span-1' },
  coldbrew: { size: 'wide', place: 'lg:col-start-2 lg:col-span-3 lg:row-start-3 lg:row-span-1' },
}
...
function DrinkCard({ drink, index, originLabel }: DrinkCardProps) {
  const { size, place } = BENTO[drink.id] ?? { size: 'small' as const, place: '' }
  ...
  return (
    <motion.article
      {...reveal((index % 3) * 0.1)}
      className={`group relative flex h-full overflow-hidden rounded-3xl border border-cream/10 bg-[linear-gradient(160deg,rgba(255,255,255,0.4),rgba(230,220,199,0.85))] p-6 transition-all duration-500 hover:border-gold/45 hover:shadow-[0_28px_70px_-24px_rgba(200,155,91,0.2)] sm:p-7 lg:p-3 ${
        isWide ? 'flex-col lg:flex-row lg:items-center lg:gap-4' : 'flex-col'
      } ${isLarge ? 'lg:justify-center' : ''} ${place}`}
    >
      ...
      <div
        className={`relative mx-auto shrink-0 transition-transform duration-500 group-hover:-translate-y-2 ${
          isLarge ? 'h-36 w-36 sm:h-44 sm:w-44 lg:h-16 lg:w-16' : isWide ? 'h-32 w-32 lg:mx-0 lg:h-12 lg:w-12' : 'h-28 w-28 lg:h-10 lg:w-10'
        }`}
      >
      ...
      <div className={`flex min-w-0 flex-1 flex-col ${isWide ? 'text-center lg:ltr:text-left lg:rtl:text-right' : 'text-center'}`}>
        <h3
          className={`text-center font-display font-medium text-cream ${
            isLarge
              ? 'mt-5 text-2xl sm:text-3xl lg:mt-1.5 lg:text-lg'
              : isWide
                ? 'mt-4 text-xl sm:text-2xl lg:mt-0 lg:text-center lg:text-sm lg:ltr:text-left lg:rtl:text-right'
                : 'mt-3 text-lg lg:mt-1 lg:text-xs'
          }`}
        >
          {drink.name}
        </h3>
        ...
        <p
          className={`font-display italic text-gold-soft transition-[letter-spacing] duration-500 group-hover:tracking-wider ${
            isLarge
              ? 'mt-4 text-center text-sm lg:mt-1 lg:text-[0.68rem]'
              : isWide
                ? 'mt-2 text-center text-sm lg:mt-0.5 lg:text-[0.62rem] lg:ltr:text-left lg:rtl:text-right'
                : 'mt-2 text-center text-xs lg:mt-0.5 lg:text-[0.6rem]'
          }`}
        >
          {drink.notes}
        </p>
        ...
        <div
          className={`mt-5 flex items-center justify-between border-t border-cream/10 pt-4 text-[0.68rem] ${
            isLarge ? 'lg:mt-1.5 lg:pt-1.5 lg:text-[0.58rem]' : 'lg:mt-1 lg:border-none lg:pt-0.5 lg:justify-center'
          }`}
        >
          <span className={isLarge ? '' : 'lg:hidden'}>
  ...
  return (
    <section id="drinks" className="relative overflow-hidden py-24 lg:py-10">
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col px-6 sm:px-8 lg:h-[calc(100svh-5rem)] lg:justify-center">
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:mt-6 lg:grid-cols-4 lg:grid-rows-3 lg:gap-3 lg:h-[46vh] lg:min-h-[360px] lg:max-h-[520px] xl:gap-3">
